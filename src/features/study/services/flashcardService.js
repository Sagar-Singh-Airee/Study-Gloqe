// src/services/flashcardService.js - 🏆 ULTIMATE ENTERPRISE EDITION 2025 ✅ FIXED
...
export const generateFlashcardsWithGemini = async (
  ...
  try {
    ...
    if (optimalCardCount < cardCount) {
      console.warn(`⚠️ Adjusting from ${cardCount} to ${optimalCardCount} cards based on content`);
    }
    ...
    // ✅ FIXED: Publish event with proper type
  ...
  try {
    ...
    const validFlashcards = flashcards
      .filter((card, index) => {
        ...
        if (!isValid) {
          console.warn(`⚠️ Skipping invalid card at index ${index}`);
        }
  ...
  try {
    ...
    const deckData = {
      ...
      analytics: {
        ...
        generatedBy: metadata.autoGenerated ? 'ai' : 'manual',
        version: '2.0'
      }
    };
...
export const reviewCard = async (deckId, cardId, quality) => {
  try {
    ...
    eventBus.publish(EVENT_TYPES.FLASHCARD_REVIEWED, {
      ...
      nextReview: sm2Result.nextReview?.toDate?.() || null,
      timestamp: new Date().toISOString()
    });
...
const updateDeckStats = async (deckId) => {
  try {
    ...
    console.warn('⚠️ Failed to update deck stats:', error);
  }
};
...
const updateDocumentFlashcardCount = async (documentId, delta) => {
  try {
    ...
    console.warn('⚠️ Failed to update document count:', error);
  }
};
