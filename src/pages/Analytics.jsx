...
// Real-time analytics data (FIXED - No infinite loop)
const useRealtimeAnalytics = (userId, timeframe = 30) => {
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        // Fetch study sessions (FIXED - removed orderBy to avoid index)
        ...
        sessionsSnap.forEach(doc => {
          ...
          if (sessionDate >= startDate.toDate()) {
            totalMinutes += session.totalTime || 0;
            sessionCount++;
        ...
        const averageSessionLength = sessionCount > 0 ? Math.round(totalMinutes / sessionCount) : 0;

        // Fetch quiz sessions (FIXED - removed orderBy)
        ...
        quizzesSnap.forEach(doc => {
          ...
          if (quizDate >= startDate.toDate()) {
            ...
            subjectPerformance[subject].scores.push(quiz.score || 0);
        ...
        const performance = Object.entries(subjectPerformance).map(([name, data]) => ({
          name,
          score: data.scores.reduce((a, b) => a + b, 0) / data.scores.length,
          quizCount: data.count
        })).sort((a, b) => b.score - a.score);

        // Weak areas (subjects with score < 70%)
        ...
        let level = 1, xp = 0, nextLevelXp = 1000, badges = [], streak = 0;
        ...
        const daysWithActivity = Object.keys(sessionsByDay).sort().reverse();
        
        let currentStreak = 0;
        const today = new Date().toDateString();
        
        if (daysWithActivity.includes(today)) {
          currentStreak = 1;
          for (let i = 1; i < daysWithActivity.length; i++) {
            const prevDay = new Date(Date.now() - i * 24 * 60 * 60 * 1000).toDateString();
            if (daysWithActivity.includes(prevDay)) {
              currentStreak++;
            } else {
              break;
            }
        ...
        setData({
          studyTime: { totalMinutes, sessionCount, averageSessionLength },
          quizPerformance: { averageScore, totalQuizzes, totalCorrect, accuracy },
  ...
  }, [userId, timeframe, refreshTrigger]); // FIXED: Only depend on these
